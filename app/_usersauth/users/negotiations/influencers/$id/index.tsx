...
import {
  useBookerInfluencerNegotiationResponse,
...
function InfluencerNegotiationDetail() {
  const { id: idParam } = Route.useParams();
  const id = Number(idParam);
  const booking = useInfluencerBooking(Number.isFinite(id) && id > 0 ? id : null);
  const respond = useBookerInfluencerNegotiationResponse();
      ...
      {!booking.isLoading && !booking.isError && b && (
        <div className="bg-white rounded-10 p-5 border">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              <div className="text-stone-500 text-sm">Influencer</div>
              <div className="font-bold">{b.influencer?.mediaName ?? "-"}</div>
            </div>
            ...
            {b.vendorCounterAmount != null ? (
              <div>
                <div className="text-stone-500 text-sm">Influencer&apos;s counter</div>
                <div className="font-bold text-amber-900">₦{b.vendorCounterAmount}</div>
              </div>
            ) : null}
          ...
          {awaitingVendor ? (
            <p className="mt-6 text-sm text-stone-600">
              Waiting for the influencer to accept your offer or send a counter-offer.
            </p>
          ) : null}
          ...
          {awaitingBooker ? (
            <div className="mt-8 space-y-4 border-t border-stone-100 pt-6">
              <p className="text-sm text-stone-700">
                The influencer proposed{" "}
                <span className="font-semibold">₦{b.vendorCounterAmount ?? "—"}</span>. Accept
                to continue to payment, or reject to close this booking.
              </p>
