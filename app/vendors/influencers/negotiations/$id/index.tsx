...
import {
  ...
  useVendorInfluencerNegotiation,
...
function VendorInfluencerNegotiationDetail() {
  const { id: idParam } = Route.useParams();
  const id = Number(idParam);
  const booking = useVendorInfluencerBooking(
    Number.isFinite(id) && id > 0 ? id : null,
  );
  const negotiate = useVendorInfluencerNegotiation();
  ...
  return (
    <section className="px-4 md:px-10 py-14 min-h-screen bg-ads360-hash">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link
          to="/vendors/influencers/negotiations"
      ...
      {!booking.isLoading && !booking.isError && b && (
        <div className="bg-white rounded-10 p-5 border">
          ...
          {b.message?.trim() ? (
            <div className="mb-6 rounded-10 border bg-stone-50 p-4">
              <div className="text-stone-500 text-sm mb-1">Post message</div>
...
export const Route = createFileRoute("/vendors/influencers/negotiations/$id/")({
