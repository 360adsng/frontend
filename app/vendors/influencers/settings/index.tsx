...
const profileSchema = z.object({
  ...
  allowNegotiation: z.enum(["true", "false"]).optional(),
...
function toE164FromParts(
  ...
  const parsed = parsePhoneNumberFromString(trimmed, countryIso2 as CountryCode);
  if (!parsed?.isValid()) return null;
  return parsed.format("E.164");
}

function InfluencerSettingsPage() {
  ...
  useEffect(() => {
    if (!me) return;
    if (me.accountType !== "influencer") return;
    ...
    setForm({
      ...
      mediaName: me.mediaName ?? "",
      address: me.address ?? "",
      bio: (me as any).bio ?? "",
      allowNegotiation: (me as any).allowNegotiation ? "true" : "false",
  ...
  const onSaveProfile = (e: React.FormEvent) => {
    e.preventDefault();
    setProfileErrors({});
    if (!me || me.accountType !== "influencer") return;
    ...
    updateProfile({
      ...
      bio: parsed.data.bio,
      allowNegotiation: parsed.data.allowNegotiation === "true",
  ...
  const canEdit = Boolean(me && me.accountType === "influencer");
      ...
      <section className="min-h-screen bg-ads360-hash px-4 md:px-10 py-14">
        {tab === "profile" ? (
          <div className="border border-ads360yellow-100 bg-white rounded-10 my-5 p-4 md:p-6">
            <h2 className="my-5 font-bold">Influencer details</h2>

            {!canEdit ? (
              <div className="text-stone-500 py-10">
                {meQuery.isLoading
                  ? "Loading..."
              ...
              <form onSubmit={onSaveProfile}>
                <div className="md:flex justify-evenly">
                  <div className="md:w-[40%] px-3">
                    ...
                    <div className="my-3">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setForm((p) => ({ ...p, mediaName: e.target.value }))
                        }
                      />
                      <FieldError message={profileErrors.mediaName} />
                    </div>
                    ...
                    <div className="my-3">
                      ...
                      <input
                        className={inputBase}
                        value={
                          me && me.accountType === "influencer"
                            ? (me.influencerType ?? "influencer")
                            : ""
                        }
                        disabled
                      />
                  ...
                  <div className="md:w-[40%] px-3">
                    <div className="my-3">
                      ...
                      <select
                        className={inputBase}
                        value={form.allowNegotiation}
                        onChange={(e) =>
                          setForm((p) => ({
                            ...p,
                            allowNegotiation: e.target.value,
                          }))
                        }
                      >
                ...
                <div className="text-center my-7">
                  <button
                    type="submit"
                    disabled={isUpdating}
                    className="group rounded-10 hover:animate-changeColor hover:text-white border bg-ads360yellow-100 py-2 px-4 disabled:opacity-60"
                  >
          ...
          <div className="border border-ads360yellow-100 bg-white rounded-10 my-5 p-4 md:p-6">
            <div className="mx-auto w-full md:w-1/2">
              <h2 className="my-2 font-bold">Change Password</h2>
...
export const Route = createFileRoute("/vendors/influencers/settings/")({
